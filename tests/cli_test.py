...
def test_bad_bossfile_causes_error():
    ...
    rc, output = run_bossy('-f', 'Bossfile.error', 'command')
    assert rc != 0
...
def test_can_run_commands_with_arguments():
    ...
    rc, output = run_bossy('hello_n', '-r', '3')
    assert rc == 0
